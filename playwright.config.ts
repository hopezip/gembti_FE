 ...
 * - 반대로 vitest는 `vite.config.ts`에서 `e2e/`를 수집하지 않도록 별도 처리한다.
 *
 * `webServer`로 next 개발 서버를 자동 기동하고, baseURL을 그 주소로 맞춘다.
...
export default defineConfig({
  ...
  // 앱(next dev 서버)을 자동 기동한다. 이미 떠 있으면 재사용한다.
  webServer: {
    command: 'pnpm dev',
    url: 'http://localhost:3000',
    reuseExistingServer: !process.env.CI,
    timeout: 120_000,
    // E2E는 항상 MSW mock으로 띄운다(로컬/CI 무관). NEXT_PUBLIC_USE_MOCK은 .env.local(gitignore)에만
