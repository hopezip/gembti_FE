...
## 작업 모드 (v2)
- **이 프로젝트의 작업 모드**: 풀팀 협업 (프론트 4 + 백엔드 4 = 8명, 본인=프론트엔드 팀원)
...
## 기술 스택 요약
- Vite + React 19 SPA (TypeScript strict) — React Router 앱 호스팅 (MIGRATE-FE-002에서 Next 제거·Vite SPA로 복귀)
- 데이터: TanStack Query
- 상태: Zustand
- UI: Panda CSS + Park UI (다크 모드·데스크탑 전용)
- 라우팅: React Router (createBrowserRouter, 라우트 SSOT 유지) — `index.html` + `src/main.tsx`로 마운트
...
- `pnpm dev` — 개발 서버 (vite, http://localhost:3000)
- `pnpm build` — 빌드 (tsc --noEmit && vite build)
- `pnpm preview` — 프로덕션 빌드 미리보기 (vite preview)
