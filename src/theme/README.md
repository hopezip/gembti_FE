...
└─ recipes/
   ...
   └─ toast.ts         # slot recipe: toast (themeExtend.slotRecipes에 등록)
```

> ⚠️ `textStyles`는 `panda.config.ts`에서 `theme.extend` **밖**(theme 직속)에 등록한다.
> 나머지(tokens/semanticTokens/recipes/slotRecipes)는 `theme.extend`에 들어간다. (`index.ts`의 `themeExtend`)
...
Park UI/Ark UI처럼 `Root`, `Title`, `Description`, `CloseTrigger` 등 여러 slot을 가진 컴포넌트는 `defineSlotRecipe`를 사용한다.

- `recipes/index.ts`의 `recipes` 객체에는 넣지 않는다.
- `src/theme/index.ts`의 `themeExtend.slotRecipes`에 등록한다.
