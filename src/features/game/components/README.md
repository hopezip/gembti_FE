...
- `GameDetailHero` — 게임 상세 Hero(REC-DET-FE-001, Figma 4014:3959). 풀블리드 커버 배경(오버레이 없이 이미지 원색 노출, 텍스트는 text-shadow로 가독 확보 — 메인 배너와 동일 패턴) 위 좌측 정렬 콘텐츠(PageContainer 1232px). 카테고리 칩 → 제목 → 별점 → 메타 3컬럼(개발/퍼블리셔·발행일·플레이모드) → 가격블록(0원=`무료`) → 구매 버튼. 배경 우선순위 thumbnailUrl→themeImageUrl→bannerUrl→bg.surfaceRaised(REC-DET-FE-004: 헤더 썸네일 우선). props: `detail`(GameDetail).
- `GameInfoTable` — 게임 정보 테이블(Figma 4014:3937). dl/dt/dd 구조(라벨 fg.subtle 고정폭 88px·값 fg.default), bg.surface 반투명 + border + radius xl. 항목: 장르/플레이 모드/연령 등급/한글 지원/최소·권장 사양. 값이 빈 항목은 `미기재`로 표시(REC-DET-FE-004). props: `detail`(GameDetail).
