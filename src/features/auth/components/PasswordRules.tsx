...
// 순수 파생 표시 컴포넌트다 — 검증의 출처(SSOT)는 signupSchema이며 여기선 같은 헬퍼로 표시만 한다.
// 규칙: 10자 이상 / 영문 포함 / 숫자 포함 = 필수, 특수문자 = 권장(선택, 미충족도 가입 가능).
...
export function PasswordRules({ value }: PasswordRulesProps) {
  const rules: Rule[] = [
    { label: '10자 이상', met: value.length >= PASSWORD_MIN_LENGTH },
