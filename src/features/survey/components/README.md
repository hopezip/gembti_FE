...
설문 관련 라우트는 모두 `Auth` 접근이며, 헤더 Nav가 아니라 메인 배너의 설문 CTA를 통해 진입한다.
...
- 문항/제출/결과 서버 상태는 `features/survey/api`의 TanStack Query 훅으로 조회한다. 진행 중 선택 상태만 로컬 훅에서 관리한다.

## 등재 컴포넌트

- `SurveyQuestionSection` — 설문 진행 화면의 feature 루트. 상단 단계 표시, 질문 문장, 1~5 선택지, 하단 컨트롤을 조립한다. 선택/건너뛰기/이전 이동은 `useSurveyQuestionSection`에 위임하고 완료 시 선택한 응답만 제출 형태로 전달한다.
...
- 대표 설명과 태그는 현재 결과 API의 6대 성향 점수 중 양수인 상위 2개 축을 기준으로 화면에서 조합한다. 백엔드가 전용 필드를 제공하면 해당 응답을 우선한다.
- 인트로/진행 화면이 공유하는 배경 스타일은 `surveyIntro.styles.ts`에서 별도 공통 파일로 분리한다.
- 별도 분석 상태 API가 확정되면 `SurveyAnalysisLoadingPage`의 표시용 진행률 타이머를 폴링 상태로 교체한다.
