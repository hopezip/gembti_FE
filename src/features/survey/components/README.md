...
survey 도메인 컴포넌트 자리. 설문 인트로(`/survey/intro`)와 설문 진행(`/survey`) 화면에서 쓰는 UI만 둔다. 다른 도메인 feature는 import하지 않는다.
...
- `Button`, `ProgressBar`처럼 공용화된 UI는 `src/components/ui`에서 가져다 쓴다. 설문 전용 모양만 feature 안에서 조정한다.
...
- 설문 완료 후 결과/추천 라우트가 확정되면 마지막 문항 이후 이동 처리를 훅 또는 service 경계에 추가한다.
